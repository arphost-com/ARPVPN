...
The following table describes every argument accepted by ARPVPN:

+------------------+------------+------------------------------------------+------------------------------------------------------+
| Argument         | Type       | Explanation                              | Notes                                                |
+==================+============+==========================================+======================================================+
| *workdir*        | Positional | Path to the ARPVPN's working directory | ARPVPN will store here everything it needs to work |
+------------------+------------+------------------------------------------+------------------------------------------------------+
| *-h* \| *--help* | Optional   | Display ARPVPN's CLI help and exit     |                                                      |
+------------------+------------+------------------------------------------+------------------------------------------------------+
...
Two sample configuration files are provided, ``uwsgi.sample.yaml`` and ``arpvpn.sample.yaml``, although the most interesting one is the second, since the first only contains options for a third party software,
`UWSGI <https://uwsgi-docs.readthedocs.io>`__.

Nonetheless, it is worth noting that the path to the ARPVPN's working directory (which will be used by ARPVPN to store stuff) needs to be provided through uwsgi's configuration, using the field ``pyargv``. Moreover, to edit the port and/or the interface in which the web server is running you will need to edit the field ``http-socket`` of uwsgi's configuration file.


For now on, we will only discuss ARPVPN's configuration values. Although the file ``arpvpn.sample.yaml``
...
.. note::
    ...
    ARPVPN will only store the **amount of bytes received and transmitted** by peers, and only if ``enabled`` is set to ``true``.
...
+----------------+------------------------------------------------------------------------------+------------------------------------------+-------------------------------------------------------------------------------------------------------------+
| *full_tunnel*  | Whether a site-to-site peer should route all traffic through ARPVPN          | ``true``, ``false``                      | Default value is ``false``. If ``true``, the downloaded peer config uses ``AllowedIPs = 0.0.0.0/0``.       |
+----------------+------------------------------------------------------------------------------+------------------------------------------+-------------------------------------------------------------------------------------------------------------+
...
Although ARPVPN stores users' credentials encrypted, it does not implement end-to-end encryption and instead, it relays on TLS to secure the communication between the user and the server.
This means you should never run ARPVPN on its own, but use the ``https`` option of uWSGI or set up a reverse proxy if you wish to use plain HTTP with uWSGI. Don't worry, here's how:
...
.. code-block:: yaml
    ...
    uwsgi:
      ...
      master: true
      enable-threads: true
      chdir: /var/www/arpvpn
      venv: venv
      wsgi-file: arpvpn/__main__.py
      pyargv: data
      need-plugin: python3
      callable: app
      die-on-term: true
