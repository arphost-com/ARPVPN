...
Project:
- Repo: `ssh://git@10.10.10.96:2424/arphost/arpvpn-multitenant.git`
- Web: `http://10.10.10.96:8929/arphost/arpvpn-multitenant`
- Registry image: `10.10.10.96:5050/arphost/arpvpn-multitenant:latest`
...
3. Set these required values in `.env`:
   ```env
   ARPVPN_IMAGE=10.10.10.96:5050/arphost/arpvpn-multitenant:latest
...
Project CI builds and publishes the standalone multitenant image to GitLab Container Registry.
...
When validating fresh multitenant behavior on `docker02`, keep only these two working clones:
...
2. `git clone` the repo back into one of those two paths.
3. Run validation from that fresh clone rather than creating extra `arpvpn-*` staging directories.

Latest docker02 standalone multitenant validation completed on 2026-03-26 for contract checks, unit subset, package build, and Docker image build.
