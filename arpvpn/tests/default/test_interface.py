...
def test_post_edit_adds_managed_local_routes(client):
    ...
    response = client.post(f"{url}/{iface.uuid}", data={
        ...
        "local_route_gateway": "192.168.1.1",
        "local_routes": "192.168.10.0/24\n192.168.1.0/24",
        "on_up": list_to_str(iface.on_up),
        "on_down": list_to_str(iface.on_down),
    })
    assert is_http_success(response.status_code)
    assert any(
        command.endswith("route replace 192.168.10.0/24 via 192.168.1.1")
        for command in iface.on_up
    )
    assert any(
        command.endswith("route replace 192.168.1.0/24 via 192.168.1.1")
        for command in iface.on_up
    )
    assert any(
        command.endswith("route del 192.168.10.0/24 via 192.168.1.1 || true")
        for command in iface.on_down
    )
    assert any(
        command.endswith("route del 192.168.1.0/24 via 192.168.1.1 || true")
        for command in iface.on_down
    )
...
def test_get_edit_prefills_managed_local_routes(client):
    login(client)
    iface = create_test_iface("iface1", "10.0.0.1/24", 50000)
    iface.on_up.append("ip route replace 192.168.10.0/24 via 192.168.1.1")
    iface.on_up.append("ip route replace 192.168.1.0/24 via 192.168.1.1")
    iface.on_down.append("ip route del 192.168.10.0/24 via 192.168.1.1 || true")
    iface.on_down.append("ip route del 192.168.1.0/24 via 192.168.1.1 || true")
    interfaces[iface.uuid] = iface
    ...
    assert b'value="192.168.1.1"' in response.data
    assert b"192.168.10.0/24" in response.data
